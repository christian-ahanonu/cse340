...
async function addToCart(req, res, next) {
    try {
        ...
        const vehicle = await invModel.getAllInventoryVehicleById(parseInt(inv_id));
...
async function removeFromCart(req, res, next) {
    try {
        ...
        if (!accountData || accountData.account_type !== "Client") {
            req.flash(
                "notice",
                "Please log in as a client to modify your cart."
            );
            return res.redirect("/account/login");
        }
        ...
        if (removedRow) {
            const vehicle = await invModel.getAllInventoryVehicleById(removedRow.inv_id);
            req.flash(
                "notice",
    ...
    try {
        ...
        req.session.orderDetails = {
            ...
            total: new Intl.NumberFormat("en-US", {
                ...
                cartItems.reduce(
                    (sum, item) =>
                        sum + parseFloat(item.inv_price) * item.quantity,
                    0
                )
            ),
        };
